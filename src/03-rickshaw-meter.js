...
export function calculateSurge(baseFare, surgeMultiplier) {
  ...
  const surge = Math.ceil(baseFare * surgeMultiplier);
  return surge;
}
...
export function getDistanceDifference(from, to) {
  // Your code here
  const fromDistace = parseInt(from);
  const toDistance = parseInt(to);

  if (Number.isNaN(fromDistace) || Number.isNaN(toDistance)) return -1;

  const distance = Math.abs(fromDistace - toDistance);
  return distance;
}
